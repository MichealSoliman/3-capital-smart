...
import { useLocale } from 'next-intl';
import { Link } from '@/lib/navigation';
import emailjs from '@emailjs/browser'; // أضف هذا الاستيراد
...
const CertificateRegistrationForm = () => {
  ...
  // تهيئة EmailJS عند تحميل المكون
  useEffect(() => {
    emailjs.init("V0cZZU860ikSX-UF_"); // استبدل هذا بالمفتاح العام الخاص بك
  }, []);
    ...
    if (validateForm()) {
      ...
      try {
        ...
        */
        ...
        // استخدم EmailJS لإرسال البيانات إلى الجيميل
    const templateParams = {
  to_name: "3Capital Team", // اسم المستقبل
  from_name: `${formData.firstName} ${formData.lastName}`, // للعرض الكامل
  firstName: formData.firstName,  // مهم جدا للـ template
  lastName: formData.lastName,    // مهم جدا للـ template
  from_email: formData.email,
  email: formData.email,          // للتأكد ان المتغير موجود في template
  phone: formData.phone,
  certificate: formData.certificate,
  certificate_name: certificates.find(c => c.id === formData.certificate)?.name || formData.certificate,
  education: formData.education,
  experience: formData.experience,
  date: new Date().toLocaleString(),
  subject: `Certificate Registration - ${formData.firstName} ${formData.lastName}`
};


        const result = await emailjs.send(
          "service_e88zysl", // Service ID من EmailJS
          "template_blglfd9", // Template ID من EmailJS
          templateParams
        );
